...
/// 获取分类后的文件列表
#[tauri::command]
pub fn get_categorized_files() -> Vec<FileInfo> {
...
pub fn organize_desktop() {
    ...
    for file in files {
        if file.is_file() {
            let category = get_file_category(&file);
            // let date_folder = get_file_date(&file);

            // 构造目标路径：桌面/分类/日期
            let target_folder = desktop_path.join(category).join("custom_files");
            if !target_folder.exists() {
                fs::create_dir_all(&target_folder).unwrap();
            }

            // 移动文件
            let target_file = target_folder.join(file.file_name().unwrap());
            if let Err(e) = fs::rename(&file, &target_file) {
